...
def collect_articles() -> list[Article]:
    ...
    for directory in ARTICLE_DIRS:
        ...
        for path in sorted(directory.glob("*.md")):
            ...
            if existing is None or (article.date, article.source_rel) > (existing.date, existing.source_rel):
                discovered[slug] = article
...
def generate_article_pages(articles: list[Article]) -> None:
    ...
    for article in articles:
        attrs: list[str] = [f"= {article.title}"]
        if article.description:
            attrs.append(f":description: {article.description}")
        if article.date:
            attrs.append(f":page-published: {article.date}")
        if article.author:
            attrs.append(f":page-author: {article.author}")
        if article.tags:
            attrs.append(f":page-tags: {', '.join(article.tags)}")
        attrs.extend([
            "",
            f"_Source markdown: `{article.source_rel}`_",
            "",
        ])
        body = "\n".join(attrs) + article.body_adoc + "\n"
        write_text(PAGES_DIR / "article" / f"{article.slug}.adoc", body)
